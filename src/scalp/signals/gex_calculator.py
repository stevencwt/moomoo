...
class GEXCalculator:
    ...
    def _compute_internal(self, ticker: str, client) -> dict:
        ...
        # Step 3: Get option chain for both calls and puts
        calls_df = client.get_option_chain(ticker, front_expiry, "CALL")
        puts_df  = client.get_option_chain(ticker, front_expiry, "PUT")

        if calls_df.empty and puts_df.empty:
            raise ValueError(f"Empty option chain for {ticker} {front_expiry}")

        # Step 4: Get snapshots with Greeks (delta, gamma, open_interest)
        call_codes = calls_df["code"].tolist() if not calls_df.empty else []
        put_codes  = puts_df["code"].tolist()  if not puts_df.empty  else []

        all_codes = call_codes + put_codes
        if not all_codes:
            raise ValueError(f"No option codes to snapshot for {ticker}")
