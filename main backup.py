...
def run_scan_now(config: dict) -> None:
    ...
    try:
        ...
        snapshots = scanner.scan_universe()
        print(f"Scanned {len(snapshots)} symbol(s):")
        for snap in snapshots:
            print(
                f"  {snap.symbol:<12} | "
                f"price=${snap.spot_price:.2f} | "
                f"regime={snap.market_regime:<8} | "
                f"IV_rank={snap.options_context.iv_rank:.0f} | "
                f"RSI={snap.technicals.rsi:.0f}"
            )

        signals = registry.evaluate_universe(snapshots)
        print(f"\n{len(signals)} signal(s) generated:")
        for sig in signals:
            print(
                f"  ✅ {sig.strategy_name:<20} | "
                f"{sig.symbol} | "
                f"credit=${sig.net_credit:.2f} | "
                f"DTE={sig.dte} | "
                f"delta={sig.delta:.2f}"
            )
        if not signals:
            print("  (no signals — entry criteria not met)")
